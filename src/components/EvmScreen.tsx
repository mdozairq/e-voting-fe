...
const EVMScreen: React.FC = () => {
  const { voter_data, candidate_data, admin_data } = useAppSelector(getAuthData);
  console.log("voter_data", voter_data);
    ...
    <div className="flex justify-center items-center h-full bg-gray-200">
      <div className="bg-white p-8 rounded shadow-md m-8 ">
        <div className="flex flex-col items-center">
          {
            voter_data && !voter_data.IsEligible ? <h2 className="text-3xl font-bold mb-6">You are not eligible to vote! (Under Age)</h2>
              : <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {candidates.map((candidate) => (
                    <div
                      key={candidate.id}
                      className={`p-6 rounded-lg cursor-pointer ${selectedCandidateId === candidate.id ? 'bg-blue-500 text-white' : 'bg-gray-100'
                        }`}
