"use client"
import { getAllElection, getElectionById } from '@/action/election';
...
const ElectionList: React.FC = () => {
    ...
    }, [all_election, dispatch]);

    return (
        <>
            {all_election && (
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-1">
                    {currentElections.map((election: any) => (
                        <div
                            key={election._id}
                            className="flex bg-white p-4 rounded shadow-md m-5 cursor-pointer items-center flex-col justify-center max-h-56 w-auto"
